...
class StrtdemPreprocessingDataset(BaseDataset):
    """
    ...
    def __init__(self, opt):
        ...
        self.transform_A = get_transform(self.opt, params=params, grayscale=(input_nc == 1), normal_params=self.dem_normal_params)
        self.transform_B = get_transform(self.opt, params=params, grayscale=(output_nc == 1), normal_params=self.strt_normal_params)
        ...
        self.transform_B_rpl = get_transform(self.opt, params=params, method=Image.NEAREST, normal_params={'need_norm': False})
        self.transform_B_wam = get_transform(self.opt, params=params, normal_params={'need_norm': False})
    ...
    def __getitem__(self, index):
        ...
        if self.avghi_loss:
            ...
            avg_height = torch.ones([1, 1, 1]) * avghi if self.average_height > 0 else torch.ones([1, 1, 1]) * torch.mean(A)
            ret['avg_height'] = avg_height

        if self.gen_height_diff:
            A_name = os.path.basename(A_path)
            row = self.df.loc[self.df['StreetMaps'] == A_name]
    ...
    def get_preinput(self):
        preinput = {'A_norm': self.dem_normal_params, 'B_norm': self.strt_normal_params}
        return preinput
